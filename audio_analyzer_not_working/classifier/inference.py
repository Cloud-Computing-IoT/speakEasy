...
def format_lines(video_ids, predictions, top_k, whitelisted_cls_mask=None):
  ...
  for video_index in range(batch_size):
    ...
    #         for class_index in range(527)]
    # line = sorted(line, key=lambda p: -p[1])
    results_dict = {}
    for class_index in range(527):
      results_dict[class_index] = predictions[video_index][class_index]
    return results_dict
    # my_str = " ".join("%i %g" % (label, score) for (label, score) in line)
    # #yield (video_ids[video_index] + "," + my_str + "\n").encode("utf8")
    # yield ("1" + "," + my_str + "\n").encode("utf8")
  ...
  with tf.Session(config=tf.ConfigProto(
    ...
    try:
      while not coord.should_stop():
        ...
        return format_lines(video_id_batch_val, predictions_val, top_k, whitelisted_cls_mask)
